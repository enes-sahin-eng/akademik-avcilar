...
function getHeightMultiplier(width: number) {
  ...
  const available = typeof window !== "undefined" ? window.innerHeight * 0.7 : 800 * 0.7; 
...
export default function CardFanCarousel({ cards }: SocialCardsProps) {
  ...
  useEffect(() => {
    if (isPaused || !needsPagination) return;
    const interval = setInterval(() => {
      cycle("right");
    }, 4000);
    return () => clearInterval(interval);
  }, [isPaused, needsPagination, cycle]);

  useEffect(() => {
    const container = containerRef.current;
    if (!container || !totalCards) return;

    const cardElements = Array.from(container.querySelectorAll<HTMLElement>(`.${styles.fanCard}`));
  ...
  return (
    ...
    >
      <div className={styles.containerWrapper}>
        <div ref={containerRef} className={styles.fanLayout}>
            ...
            const image = (
              <div className={styles.imageWrapper}>
                <Image 
                  ...
                  unoptimized={card.imgUrl.startsWith("http")}
                />
              </div>
            );
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className={styles.fanCard}>
                {image}
              </a>
            ) : (
              <div key={index} className={styles.fanCard}>
                {image}
              </div>
            );
          })}
      ...
      {needsPagination && (
        <div className={styles.pagination}>
          <button className={styles.arrowBtn} onClick={() => cycle("left")} aria-label="Önceki">
            {chevron("left")}
          </button>
          ...
          <button className={styles.arrowBtn} onClick={() => cycle("right")} aria-label="Sonraki">
            {chevron("right")}
          </button>
        </div>
      )}
