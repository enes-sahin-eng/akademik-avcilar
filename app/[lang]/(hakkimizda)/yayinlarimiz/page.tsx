...
const COURSE_KEY = "yayinlarimizPage";
const PAGE_SLUG = "yayinlarimiz";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const meta = (dict as any)?.[COURSE_KEY]?.meta || {
    title: "Yayınlarımız | Akademik International",
    description: "The Academic Publishing yayınevimiz tarafından hazırlanan yabancı dil eğitim kitapları.",
  };
...
export default async function YayinlarimizPage({ params }: PageProps) {
  const { lang: rawLang } = await params;
  if (!locales.includes(rawLang as Locale)) notFound();
  const lang = rawLang as Locale;
  const dict = await getDictionary(lang);
  ...
  const publicationsName = lang === "en" ? "Publications" : lang === "ar" ? "منشوراتنا" : "Yayınlarımız";
