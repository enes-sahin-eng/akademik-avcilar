...
export async function generateMetadata({
  ...
  const meta = (dict as any)?.yokdilLandingPage?.meta || {
    ...
    description:
      lang === "en"
        ...
        : lang === "ar"
          ? "استعد لامتحان YÖKDİL مع مدربين خبراء متخصصين في مجالات الصحة والعلوم والعلوم الاجتماعية."
          : "Sosyal, Fen ve Sağlık Bilimleri alanlarına özel hazırlanan YÖKDİL eğitim programımız ve uzman eğitmen kadromuzla hedeflediğiniz puana ulaşın.",
  };
  ...
  return {
    ...
    alternates: {
      canonical: `/${lang}/yokdil-hazirlik-kursu`,
    },
    openGraph: {
      title: meta.title,
      description: meta.description,
...
export default async function YokdilHazirlikKursuPage({ params }: PageProps) {
  const { lang: rawLang } = await params;
  const lang = (locales.includes(rawLang as Locale) ? rawLang : "tr") as Locale;
  const dict = await getDictionary(lang);

  // Dinamik SSS Verisi (JSON-LD için)
  const faqItems = (dict as any)?.yokdilLandingPage?.faq?.items || [];
  ...
  return (
    <main>
          ...
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              },
              {
                "@type": "ListItem",
                position: 3,
                name: "YÖKDİL Hazırlık Kursu",
                item: `https://www.avcilaringilizcekursu.com.tr/${lang}/yokdil-hazirlik-kursu`,
              },
            ],
          }),
        }}
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            name: "YÖKDİL Hazırlık Kursu",
            description:
              "Sosyal Bilimler, Fen Bilimleri ve Sağlık Bilimleri alanlarına özel hazırlanan, yüksek lisans, doktora, doçentlik ve akademik atama hedeflerine yönelik YÖKDİL hazırlık programı.",
            provider: {
      ...
      <CourseFAQ courseKey="yokdilLandingPage" lang={lang} />
      <SeoContentBlock courseKey="yokdilLandingPage" lang={lang} />
