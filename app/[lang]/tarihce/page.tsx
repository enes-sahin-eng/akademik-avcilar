...
const COURSE_KEY = "tarihcePage";
const PAGE_SLUG = "tarihce";
...
export default async function TarihcePage({ params }: PageProps) {
  const { lang: rawLang } = await params;
  if (!locales.includes(rawLang as Locale)) notFound();
  const lang = rawLang as Locale;
  const dict = await getDictionary(lang);
  ...
  const pageName = lang === "en" ? "History" : lang === "ar" ? "تاريخنا" : "Tarihçe";
