...
const COURSE_KEY = "izmirIngilizceLandingPage";
const PAGE_SLUG = "izmir-ingilizce-dil-kursu";
...
export default async function IzmirIngilizceKursuPage({ params }: PageProps) {
  const { lang: rawLang } = await params;
  if (!locales.includes(rawLang as Locale)) notFound();
  const lang = rawLang as Locale;
  const dict = await getDictionary(lang);
