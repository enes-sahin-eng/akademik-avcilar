...
const COURSE_KEY = "ankaraIngilizceLandingPage";
const PAGE_SLUG = "ankara-ingilizce-dil-kursu";
...
export default async function AnkaraIngilizceKursuPage({ params }: PageProps) {
  const { lang: rawLang } = await params;
  if (!locales.includes(rawLang as Locale)) notFound();
  const lang = rawLang as Locale;
  const dict = await getDictionary(lang);
