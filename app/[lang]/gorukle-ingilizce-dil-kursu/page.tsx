...
const COURSE_KEY = "gorukleIngilizceLandingPage";
const PAGE_SLUG = "gorukle-ingilizce-dil-kursu";
...
export default async function GorukleIngilizceKursuPage({ params }: PageProps) {
  const { lang: rawLang } = await params;
  if (!locales.includes(rawLang as Locale)) notFound();
  const lang = rawLang as Locale;
  const dict = await getDictionary(lang);
